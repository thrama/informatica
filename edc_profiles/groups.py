...
class Groups:
    ...
    def createNew(group, groupDomain):
        ...
        jsonData = {"memberName":groupDomain+"\\"+group,"memberType":"GROUP","permissions":[{"resourceName":"DataDomain","classFilters":[{"className":"core.IClass","permission":"READ_WRITE","canReadData":False}]},{"resourceName":"DataDomainGroup","classFilters":[{"className":"core.IClass","permission":"READ_WRITE","canReadData":False}]}]}
    ...
    def appendPermission(jsonOrig, jsonPerms):
        ...
        try:       
            ...
            print(f"Successfully added the permissions for the resource [{jsonPerms['resourceName']}] in group [{jsonOrig['memberName']}].")
            logging.info(f"Successfully added the permissions for the resource [{jsonPerms['resourceName']}] in group [{jsonOrig['memberName']}].") # log

            #print(response.json)
            return response.status_code

        except ConnectionError: # Connectio Error
            print(f"RestAPI connection error for the resource [{jsonPerms['resourceName']}] in function [appendPermission].")
            logging.error(f"RestAPI connection error for the resource [{jsonPerms['resourceName']}] in function [appendPermission].") # log
    ...
    def modifyPermission(jsonOrig, jsonPerms, position):
        ...
        try:       
            ...
            print(f"Successfully updated the permissions for the resource [{jsonPerms['resourceName']}] in group [{jsonOrig['memberName']}].")
            logging.info(f"Successfully updated the permissions for the resource [{jsonPerms['resourceName']}] in group [{jsonOrig['memberName']}].") # log

            #print(response.json)
            return response.status_code

        except ConnectionError: # Connectio Error
            print(f"RestAPI connection error for the resource [{jsonPerms['resourceName']}] in function [modifyPermission].")
            logging.error(f"RestAPI connection error for the resource [{jsonPerms['resourceName']}] in function [modifyPermission].") # log
