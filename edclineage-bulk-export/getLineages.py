...
from props.utils import HTTPAdapter, Retry, datetime, logging, os, pd, requests
...
def getLineagesEdc():
    ...
    resp1 = requests.get(LINEAGE_REST_1, headers=EDC_headers, auth=EDC_Auth)
    ...
    if resp1.status_code == 200:
        ...
        if newTotal > pageSeizeOffset:
            while newTotal > pageSeizeOffset:
                ...
                offsetUrl = (
                    f"/access/2/catalog/data/search?basicQuery=*"
                    f"{basicQueryFilter}"
                    f"&tabId=all&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.profiling.DataDomain%22&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22DataDomain%22&fq="
                    f"{ambienteFilter}"
                    f"&defaultFacets=true&highlight=false&offset="
                    f"{str(pageSeizeOffset)}"
                    f"&pageSize="
                    f"{str(pageSize)}"
                    f"&includeRefObjects=true&fq=-com.infa.ldm.axon.status:Deleted"
                )
        ...
        if csvLineageColumns:
            ...
            downloaded += 1
            df.to_csv(os.path.join("EDC/LINEAGE_x_COLUMN.csv"), sep=";", index=False)

        if not csvLineageColumns:
            csvLineageColumns.append({"id": "", "outResource": "", "outId": "", "inResource": "", "inId": ""})
            df = pd.DataFrame(csvLineageColumns)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/LINEAGE_x_COLUMN.csv"), sep=";", index=False)
                ...
                if not resp.content:
                    ...
                    if newTotal >= pageSeizeOffset:
                        while totalPage >= pageSeizeOffset:
                            ...
                            logging.debug(f"Pagination Lineage: Offset items: -- {pageSeizeOffset} ")
                            offsetUrl = (
                                "/access/2/catalog/data/search?basicQuery="
                                + nameUrl
                                + "&tabId=all&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Column%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.ViewColumn%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22DataFile.Custom.Upload.FilesColumnName%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22core.ReferenceDataElement%22&facet=false&defaultFacets=true&highlight=false&offset="
                                + str(pageSeizeOffset)
                                + "&pageSize="
...
def generateColumns(childId):
    ...
    lineageColumnUrl = (
        EDC_URL_REST_2
        + "/2/catalog/data/relationships?association=core.DataFlow&depth=0&direction=IN&includeRefObjects=true&includeTerms=false&removeDuplicateAggregateLinks=true&seed="
        + childId
    )

    # print("DEBUG lineageColumnUrl:",lineageColumnUrl)
    lineageColumnReq = session.get(lineageColumnUrl, headers=EDC_headers, auth=EDC_Auth)
