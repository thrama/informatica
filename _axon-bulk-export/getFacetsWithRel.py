from props.utils import os, requests, json, time, datetime, dt_string, pd, logging
from facetOptionsWithRel import facets
from db.database import axon_with_relations, dbGeneral
from props.paramsAxon import facetPageLimit, relationsPageLimit, chars_to_remove
...
def relationFacets(baseurl, infaTokenValue, headerValue, sessionCookies):
    ...
    for facet in facets:
        ...
        if r.status_code == 200:
            ...
            exportRel = createArray(name, relName,
                                    mainItems, getRelatedItems, exportedItems)
            ...
            while totalHitsRel > limitValueRel:
                ...
                if r2.status_code == 200:
                    ...
                    exportRel = createArray(name, relName,
                                            offsetItems, relatedObjects2, exportedItems)
            ...
            while totalHitsMain > limitValueMain:
                ...
                # send request with authorization and body data
                rMain = requests.post(baseurl, headers=headerValue,
                                      verify=False, cookies=sessionCookies, stream=True, data=bodyData2)
                ...
                if rMain.status_code == 200:
                    ...
                    exportRel = createArray(name, relName,
                                            mainItems3, getRelatedItems2, exportedItems)
                    ...
                    while totalHitsRelSec > limitValueRelSec:
                        ...
                        r4 = requests.post(baseurl, headers=headerValue,
                                           verify=False, cookies=sessionCookies, stream=True, data=newFacetRel)
                        ...
                        if r4.status_code == 200:
                            ...
                            # get relatedObjects with items
                            relatedObjects3 = json_resp4.get(
                                "relatedObjects")
                            offsetItems3 = mainObject4.get(
                                "items")  # get items

                            exportRel = createArray(name, relName,
                                                    offsetItems3, relatedObjects3, exportedItems)
            ...
            for i in chars_to_remove:
                ...
                df.to_csv(os.path.join(
                            '../AXON/'+name+'_x_'+relName+'.csv'), sep = ';', index=False)
        ...
        axon_with_relations(str(name+'_x_'+relName), response_code, response_size,
                ...
                for y in rel:
                    ...
                    for val in relation:
                            ...
                            if 'relDirection' in val and val['relDirection'] == "INBOUND":
                                logging.debug(f"Relation with ID {val['id']} and direction {val['relDirection']} is skypped.")
                                continue
