from props.utils import os, pd, requests, HTTPAdapter, BeautifulSoup, json, datetime, dt_string, parser, logging
from props.paramsEdc import EDC_URL_REST_1_DATAFILE, EDC_URL_REST_1_DATAFILE, EDC_URL_REST_2, EDC_headers, EDC_Auth, pageSize, EDC_URL, chars_to_remove, val_DGR
    ...
    if resp1.status_code == 200:
                ...
                if respWhile.status_code == 200:
                    ...
                    for val_hits in hits:
                        ...
                        for val_values in values_hits:
                            if val_values.get('attributeId') == 'core.resourceName':
                                resourceName = val_values.get('value')
                        Last_Run_write[resourceName] = datetime.now().strftime(
                            "%Y-%m-%dT%H:%M:%S")
        ...
        if csvFilesPath_data:
            ...
            df.to_csv(os.path.join(
                'EDC/DATAFILE_PATHS.csv'), sep=";", index=False)
            downloaded += 1
        else:
            df = pd.DataFrame(csvFilesPathEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATAFILE_PATHS.csv'), sep=";", index=False, header=True)
            ...
            for column in df.columns:
                if column != "path id" and column != "id" and column != "Name":
                    ...
                    for i in chars_to_remove:
                        df = df.applymap(lambda x: x.replace( i, '') if (isinstance(x, str)) else x)
            df.to_csv(os.path.join(
                'EDC/DATAFILE_FILES.csv'), sep=";", index=False)
            downloaded += 1
        else:
            df = pd.DataFrame(csvFilesFileNameEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATAFILE_FILES.csv'), sep=";", index=False, header=True)
            ...
            for column in df.columns:
                if column != "File id" and column != "id" and column != "name":
                    ...
                    for i in chars_to_remove:
                        df = df.applymap(lambda x: x.replace( i, '') if (isinstance(x, str)) else x)
            df.to_csv(os.path.join(
                'EDC/DATAFILE_FILE_COLUMNS.csv'), sep=";", index=False)
            downloaded += 1
        else:
            df = pd.DataFrame(csvFilesColumnEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATAFILE_FILE_COLUMNS.csv'), sep=";", index=False, header=True)
        ...
        if not Last_Run_read.get(resourceName):
                        ...
                        while totalCount > pageSeizeOffset:
                            ...
                            if newResp.status_code == 200:
                                ...
                                if lastScanStatus != 'SUCCESS':
                                    ...
                                    genAllReferencesCSVs(hits,
                                                    EDC_headers, EDC_Auth)
                            else:
                                logging.error(f"Error rdbms pagination on offset: {pageSeizeOffset}")
                            pageSeizeOffset += pageSize

                    else:
                        print('Error generating single resource ...', )
        else:
            ...
            if fromDate < toDate:
                ...
                if lastScanStatus != 'SUCCESS':
                    ...
                    singleURL = EDC_URL_REST_2 + '/2/catalog/data/search?basicQuery=*&tabId=all&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22' + resourceName + '%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22DataFile.Custom.Upload.FilesPath%22&facet=false&defaultFacets=true&highlight=false&offset=0&pageSize='+str(pageSize)+'&includeRefObjects=true&fq=-com.infa.ldm.axon.status:Deleted'
                    ...
                    if resp2.status_code == 200:
                        ...
                        if newTotal > pageSeizeOffset:
                            while totalCount > pageSeizeOffset:
                                ...
                                offsetUrl = '/2/catalog/data/search?basicQuery=*&tabId=all&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22' + resourceName + '%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22DataFile.Custom.Upload.FilesPath%22&facet=false&defaultFacets=true&highlight=false&offset='+str(pageSeizeOffset)+'&pageSize='+str(pageSize)+'&includeRefObjects=true&fq=-com.infa.ldm.axon.status:Deleted'
