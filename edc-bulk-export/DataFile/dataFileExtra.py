...
from props.utils import BeautifulSoup, HTTPAdapter, Retry, datetime, json, logging, os, pd, requests, toDateEdc
...
def datafileResourcesExtra():
    ...
    if resp1.status_code == 200:
        ...
        if csvFilesPath_data:
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/DATAFILE_PATHS_EXTRA.csv"), sep=";", index=False, header=True)
        ...
        if csvFilesFileName_data:
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/DATAFILE_FILES_EXTRA.csv"), sep=";", index=False, header=True)
        ...
        if csvFilesColumn_data:
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/DATAFILE_FILE_COLUMNS_EXTRA.csv"), sep=";", index=False, header=True)
...
def pushToArray(resourceName):
    ...
    Last_Run_write[resourceName] = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if Last_Run_read.get(resourceName) == None:
        ...
        for v in valHits:
            ...
            if v["attributeId"] == "core.classType" and v["value"] == "DataFile.Custom.Upload.FilesColumnName":
                ...
                FilesColumnNameReq = session.get(FilesColumnNameURL, headers=EDC_headers, auth=EDC_Auth)
                # handle other errors
                if FilesColumnNameReq.status_code == 200:
                    if not FilesColumnNameReq.content:
                        ...
                        genFilesColumnCSV(FilesColumnNameRes, resourceName)
