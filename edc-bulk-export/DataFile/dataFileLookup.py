import csv
from props.utils import os, pd, requests, HTTPAdapter, json, time, now, BeautifulSoup, Retry, datetime, logging
from props.paramsEdc import EDC_URL_REST_1_DATAFILE, EDC_URL_REST_2, url_lookup_childId, EDC_headers, EDC_Auth, pageSize, EDC_URL, chars_to_remove, csv_path, val_DGR
...
def dataFileLookup():
    ...
    resp1 = session.get(EDC_URL_REST_1_DATAFILE,
    ...
    if resp1.status_code == 200:
        ...
        if csvLookup_data:
            ...
            df.to_csv(os.path.join(
                'EDC/DATAFILE_LOOKUP.csv'), sep=";", index=False)
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATAFILE_LOOKUP.csv'), sep=";", index=False, header=True)
            ...
            for column in df.columns:
                if column != "File id" and column != "id" and column != "name":
                    ...
                    for i in chars_to_remove:
                        df = df.applymap(lambda x: x.replace( i, '') if (isinstance(x, str)) else x)
            output_path = os.path.join(
                csv_path + '/DATAFILE_FILE_COLUMNS.csv')
            df.to_csv(output_path, mode='a', sep=";", index=False,
                      header=not os.path.exists(output_path))
            downloaded = downloaded + 1
        ...
        file_path = os.path.join(
            script_dir, 'DATAFILE_LOOKUP_last_updates.json')
...
def genLookupCSV(hits):
    ...
    for items in hits:
        if items['classType'] == 'DataFile.Custom.Upload.FilesFileName'or items['classType'] == 'DataFile.Custom.Upload.FilesColumnName':
