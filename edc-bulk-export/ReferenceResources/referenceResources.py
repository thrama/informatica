...
from .EmptyCsvReferenceToExport import csvDataElementEmpty, csvDatasetEmpty, csvDatasourceEmpty
...
def genReferencesFile():
    ...
    resp1 = requests.get(EDC_URL_REST_1_REFERENCE, headers=EDC_headers, auth=EDC_Auth)
    # db data to be saved
    ...
    if resp1.status_code == 200:
        ...
        if newTotal > pageSeizeOffset:
            while totalPage > pageSeizeOffset:
                ...
                offsetUrl = (
                    "/access/2/catalog/data/search?basicQuery=*&tabId=tab.resources&fq=%7B!tag%3Dresource_type%7Dcore.resourceType%3A%22Reference%5C%20resource%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22core.Resource%22&fq="
                    + val_DGR
        ...
        if csvDataset_data:
            ...
            df.to_csv(os.path.join("EDC/REFERENCE_DATASET.csv"), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            df = pd.DataFrame(csvDatasetEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/REFERENCE_DATASET.csv"), sep=";", index=False, header=True)
        ...
        if csvDatasource_data:
            ...
            for column in df.columns:
                ...
                for i in chars_to_remove:
                    df = df.applymap(lambda x: x.replace(i, "") if (isinstance(x, str)) else x)
            df.to_csv(os.path.join("EDC/REFERENCE_DATASOURCE.csv"), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            df = pd.DataFrame(csvDatasourceEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/REFERENCE_DATASOURCE.csv"), sep=";", index=False, header=True)
        ...
        if csvDataElement_data:
            ...
            for column in df.columns:
                ...
                for i in chars_to_remove:
                    df = df.applymap(lambda x: x.replace(i, "") if (isinstance(x, str)) else x)
            df.to_csv(os.path.join("EDC/REFERENCE_DATAELEMENT.csv"), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            df = pd.DataFrame(csvDataElementEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/REFERENCE_DATAELEMENT.csv"), sep=";", index=False, header=True)
...
def generateCSV(hits, EDC_headers, EDC_Auth):
    ...
    for val_hits in hits:
        ...
        singleURL = (
            EDC_URL_REST_2
            + "/2/catalog/data/search?basicQuery=*&tabId=all&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22core.ReferenceDataSource%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22core.ReferenceDataSet%22&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22"
            + resourceName
            + "%22&facet=false&defaultFacets=true&highlight=false&offset=0&pageSize="
            + str(pageSize)
            + "&includeRefObjects=true&fq=-com.infa.ldm.axon.status:Deleted"
        )
        ...
        if resp2.status_code == 200:
            ...
            if newTotal > pageSeizeOffset:
                while totalCount > pageSeizeOffset:
                    offsetUrl = (
                        "/access/2/catalog/data/search?basicQuery=*&tabId=all&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22core.ReferenceDataSource%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22core.ReferenceDataSet%22&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22"
                        + resourceName
        ...
        if not val:
            ...
            for v in valHits:
                if v["attributeId"] == "core.classType" and v["value"] == "core.ReferenceDataSource":
                    ...
                    ReferenceDataSourceReq = session.get(ReferenceDataSource, headers=EDC_headers, auth=EDC_Auth)
                ...
                if v["attributeId"] == "core.classType" and v["value"] == "core.ReferenceDataSet":
                    ...
                    datasetReq = session.get(datasetURL, headers=EDC_headers, auth=EDC_Auth)
                        ...
                        if not datasetReq.content:
                            ...
                            for val in dstLinksTb:
                                if val["classType"] == "core.ReferenceDataElement":
                                    ...
                                    dataset_dstReq = session.get(dataset_dstURL, headers=EDC_headers, auth=EDC_Auth)
                                    # handle other errors
                                    if dataset_dstReq.status_code == 200:
                                        if not dataset_dstReq.content:
                                            ...
                                            genDataElementCSV(dataElementRes)
...
def genDataSourceCSV(ReferenceDataSourceRes):
    ...
    for val in dataSourcesFacts:
        ...
        if val.get("attributeId") == "core.lastModified":
            DS_lastModified = val.get("value")
        if val.get("label") == "GDPR Data Processing note":
            DS_gdprDataProc = val.get("value")
        if val.get("label") == "GDPR Data Processing note":
            DS_gdprDataProcModifiedBy = val.get("modifiedBy")
        if val.get("label") == "Architectural Level":
            DS_architecturalLevel = val.get("value")
        if val.get("label") == "Architectural Level":
            DS_architecturalLevelModifiedBy = val.get("modifiedBy")
        if val.get("label") == "Business Description":
            DS_businessDescription = val.get("value")
        if val.get("label") == "Business Description":
            DS_businessDescriptionModifiedBy = val.get("modifiedBy")
        if val.get("attributeId") == "core.resourceName":
            DS_resourceName = val.get("value")
        if val.get("attributeId") == "core.resourceType":
            DS_resourceType = val.get("value")
...
def genDatasetCSV(datasetRes):
    ...
    for val in datasetFacts:
        if val.get("attributeId") == "core.lastModified":
            D_set_lastModified = val.get("value")
        if val.get("label") == "GDPR Data Processing note":
            D_set_gdprDataProc = val.get("value")
        if val.get("label") == "GDPR Data Processing note":
            D_set_gdprDataProcmodifiedBy = val.get("modifiedBy")
        if val.get("label") == "Data Quality Link":
            D_set_DataQualityLink = val.get("value")
        if val.get("label") == "Data Quality Link":
            D_set_DataQualityLinkModifiedBy = val.get("modifiedBy")
        if val.get("label") == "Business Description":
            D_set_businessDescription = val.get("value")
        if val.get("label") == "Business Description":
            D_set_businessDescriptionModifiedBy = val.get("modifiedBy")
        if val.get("attributeId") == "core.resourceName":
            D_set_resourceName = val.get("value")
        if val.get("attributeId") == "core.name":
...
def genDataElementCSV(dataElementRes):
    ...
    for val in dataElementFacts:
        ...
        if val.get("label") == "Data Quality Link":
            DE_DataQualityLinkModifiedBy = val.get("modifiedBy")
        if val.get("label") == "Asset Classification":
            DE_AssetClassification = val.get("value")
        if val.get("label") == "Asset Classification":
            DE_AssetClassificationModifiedBy = val.get("modifiedBy")
        if val.get("label") == "Business Description":
            DE_businessDescription = val.get("value")
        if val.get("label") == "Business Description":
            DE_businessDescriptionModifiedBy = val.get("modifiedBy")
        if val.get("attributeId") == "core.resourceName":
