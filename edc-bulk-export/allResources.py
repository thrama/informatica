...
def genAllResourcesFile():
    ...
    resp1 = session.get(EDC_URL_REST_1, headers=EDC_headers, auth=EDC_Auth)
        ...
        if resp2.status_code == 200:
            if not resp2.content:
                ...
                for val in facts:
                    ...
                    if val.get("label") == "Architectural Level":
                        architecturalLevelModifiedBy = val.get("modifiedBy")
                    ...
                    if val.get("label") == "Business Description":
                        businessDescriptionModifiedBy = val.get("modifiedBy")
                ...
                df.to_csv(os.path.join("EDC/RESOURCES.csv"), sep=";", index=False)
