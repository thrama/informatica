from props.utils import os, pd, requests, HTTPAdapter, json, time, BeautifulSoup, Retry, datetime, logging
from props.paramsEdc import EDC_URL_REST_1_RELATIONAL, EDC_URL_REST_2, url_lookup_childId, EDC_headers, EDC_Auth, pageSize, EDC_URL, chars_to_remove, val_DGR, csv_path
    ...
    if resp1.status_code == 200:
        ...
        if csvLookup_data:
            ...
            df.to_csv(os.path.join(
                'EDC/RDBMS_LOOKUP.csv'), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            # Generate empty csv files csvLookup
            df = pd.DataFrame(csvLookupEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join('EDC/RDBMS_LOOKUP.csv'),
        ...
        if csvTable_data:
            ...
            output_path=os.path.join(
                csv_path +'/RDBMS_TABLE VIEWS.csv')
...
def generateCSV(hits, Last_Run_read, EDC_headers, EDC_Auth):
    ...
    for val_hits in hits:
        ...
        checkLastRun = datetime.fromtimestamp(
            Last_Run/1000).strftime('%Y-%m-%dT%H:%M:%S')
        ...
        if lastScanStatus != 'SUCCESS' or lastScan < checkLastRun:
            ...
            if Last_Run_read.get(resourceName) == None:
                ...
                singleURL = EDC_URL_REST_2 + '/2/catalog/data/app_events?resourceName=' + resourceName + \
                    '&offset=0&pageSize=' + str(pageSize) + '&sortBy=createdOn&responseType=DETAILED&changeType=SOURCE&since=' + \
                    str(fromDate)
                resp2 = session.get(
                ...
                if resp2.status_code == 200:
                    ...
                    if newTotal > pageSeizeOffset:
                        while totalCount > pageSeizeOffset:
                            offsetUrl = '/2/catalog/data/app_events?resourceName=' + resourceName + '&offset=' + str(pageSeizeOffset)+'&pageSize=' + str(
                                pageSize) + '&sortBy=createdOn&responseType=DETAILED&changeType=SOURCE&since=' + str(fromDate)
                            ...
                            if newResp.status_code == 200:
                                ...
                                print("Error rdbms lookup pagination on offset: ",
                                    pageSeizeOffset)
                            pageSeizeOffset += pageSize
        ...
        if newResourceFailed == 0 :
           newDate = datetime.utcnow().strftime(
                "%Y-%m-%d %H:%M:%S")
...
def genLookupCSV(hits):
    ...
    for items in hits:
        if (items['objectLabel'] == 'Table') or (items['objectLabel'] == 'View') or (items['objectLabel'] == 'Column') or (items['objectLabel'] == 'ViewColumn'):
...
def genTabRdbmsCSVs(childId, EDC_headers, EDC_Auth):
    ...
    lookupReq = requests.get(url_lookup, headers=EDC_headers, auth=EDC_Auth, verify=False)
        ...
        for v in valHits:
            ...
            if (v['attributeId'] == 'core.classType' and v['value'] ==  'com.infa.ldm.relational.Column') or (v['attributeId'] == 'core.classType' and v['value'] ==  'com.infa.ldm.relational.ViewColumn'):
                ...
                columReq = session.get(
                    columUrl, headers=EDC_headers, auth=EDC_Auth, verify=False)
                # handle other errors
