...
from props.utils import BeautifulSoup, HTTPAdapter, Retry, datetime, json, logging, os, parser, pd, requests

from .EmptyCsvRdbmsToExport import csvColumnEmpty, csvDatabaseEmpty, csvSchemaEmpty, csvTableEmpty
...
def rdbmsResourcesFile():
    ...
    if resp1.status_code == 200:
        ...
        if csvDatabase_data:
            ...
            df.to_csv(os.path.join("EDC/RDBMS_DATABASES.csv"), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            df = pd.DataFrame(csvDatabaseEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/RDBMS_DATABASES.csv"), sep=";", index=False, header=True)
        ...
        if csvSchema_data:
            ...
            df.to_csv(os.path.join("EDC/RDBMS_SCHEMA.csv"), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            df = pd.DataFrame(csvSchemaEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/RDBMS_SCHEMA.csv"), sep=";", index=False, header=True)
        ...
        if csvTable_data:
            ...
            df.to_csv(os.path.join("EDC/RDBMS_TABLE VIEWS.csv"), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            df = pd.DataFrame(csvTableEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/RDBMS_TABLE VIEWS.csv"), sep=";", index=False, header=True)
        ...
        if csvColumn_data:
            ...
            df.to_csv(os.path.join("EDC/RDBMS_COLUMNS.csv"), sep=";", index=False)
            downloaded = downloaded + 1
        else:
            df = pd.DataFrame(csvColumnEmpty)
            df = df.iloc[0:-1]
            df.to_csv(os.path.join("EDC/RDBMS_COLUMNS.csv"), sep=";", index=False, header=True)
        ...
        if not Last_Run_read.get(resourceName):
                ...
                if resp2.status_code == 200:
                    ...
                    if newTotal > pageSeizeOffset:
                        while totalCount > pageSeizeOffset:
                            ...
                            offsetUrl = (
                                "/access/2/catalog/data/search?basicQuery=*&tabId=all&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Database%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Schema%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Table%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.View%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Column%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.ViewColumn%22&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22"
                                + resourceName
            ...
            if fromDate < toDate:
                ...
                if lastScanStatus != "SUCCESS":
                    ...
                    singleURL = (
                        EDC_URL_REST_2
                        + "/2/catalog/data/search?basicQuery=*&tabId=all&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Schema%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Database%22&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22"
                        + resourceName
                        + "%22&facet=false&defaultFacets=true&highlight=false&offset=0&pageSize="
                        + str(pageSize)
                        + "&includeRefObjects=true&fq=-com.infa.ldm.axon.status:Deleted"
                    ...
                    if resp2.status_code == 200:
                        ...
                        if newTotal > pageSeizeOffset:
                            while totalCount > pageSeizeOffset:
                                ...
                                logging.info(f"Working on {resourceName} [{pageSeizeOffset} of {totalCount}]")
                                offsetUrl = (
                                    "/access/2/catalog/data/search?basicQuery=*&tabId=all&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Schema%22&fq=%7B!tag%3Dall_class_type%7Dcore.allclassTypes%3A%22com.infa.ldm.relational.Database%22&fq=%7B!tag%3Dresource_name%7Dcore.resourceName%3A%22"
                                    + resourceName
                                    + "%22&facet=false&defaultFacets=true&highlight=false&offset="
                                    + str(pageSeizeOffset)
                                    + "&pageSize="
        ...
        if val not in hits:
            ...
            for v in valHits:
                if v["attributeId"] == "core.classType" and v["value"] == "com.infa.ldm.relational.Database":
                    ...
                    databaseReq = session.get(databaseURL, headers=EDC_headers, auth=EDC_Auth)
                    # handle other errors
                ...
                if v["attributeId"] == "core.classType" and v["value"] == "com.infa.ldm.relational.Schema":
                    ...
                    schemaReq = session.get(schemaURL, headers=EDC_headers, auth=EDC_Auth)
                ...
                if (v["attributeId"] == "core.classType" and v["value"] == "com.infa.ldm.relational.Column") or (
                    ...
                    columReq = session.get(columnURL, headers=EDC_headers, auth=EDC_Auth)
                    # handle other errors
