from props.utils import os, pd, requests, HTTPAdapter, BeautifulSoup, Retry, json, dt_string, datetime, logging
from props.paramsEdc import EDC_URL_REST_1_DATADOMAIN, EDC_URL_REST_1, EDC_URL_REST_2, EDC_headers, EDC_Auth, pageSizeDD, EDC_URL, val_DGR, chars_to_remove
...
def customDatadomainFile():
    ...
    resp1 = requests.get(EDC_URL_REST_1_DATADOMAIN,
    ...
    if resp1.status_code == 200:
        ...
        if csvDataDomainGroup:
            ...
            df.to_csv(os.path.join(
                'EDC/DATADOMAINGROUPS.csv'), sep=";", index=False)
        ...
        if csvDataDomainGroup_X_DataDomain:
            ...
            df.to_csv(os.path.join(
                'EDC/DATADOMAINGROUPS_x_DATADOMAIN.csv'), sep=";", index=False)
            downloaded = downloaded + 1
            ...
            for column in df.columns:
                ...
                for i in chars_to_remove:
                    df = df.applymap(lambda x: x.replace(
                        i, '') if (isinstance(x, str)) else x)
            df.to_csv(os.path.join(
                'EDC/DATADOMAINS.csv'), sep=";", index=False)
            downloaded = downloaded + 1
        ...
        if csvDataDomain_X_Column:
            ...
            #df.drop_duplicates (inplace=True)
            df.to_csv(os.path.join(
                'EDC/DATADOMAIN_x_COLUMN.csv'), sep=";", index=False)
            downloaded = downloaded + 1
        ...
        if not csvDataDomainGroup:
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATADOMAINGROUPS.csv'), sep=";", index=False, header=True)

        if not csvDataDomainGroup_X_DataDomain:
            csvDataDomainGroup_X_DataDomain.append({
                'id': '',
                'Name': '',
                'DataDomain id': ''
            })
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATADOMAINGROUPS_x_DATADOMAIN.csv'), sep=";", index=False, header=True)

        if not csvDataDomain_X_Column:
            csvDataDomain_X_Column.append({
                "id": '',
                'Name': '',
                'Column id': ''
            })
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATADOMAIN_x_COLUMN.csv'), sep=";", index=False, header=True)
        ...
        if not csvDataDomain:
            ...
            df = df.iloc[0:-1]
            df.to_csv(os.path.join(
                'EDC/DATADOMAINS.csv'), sep=";", index=False, header=True)
        ...
        if not val:
            ...
            for v in valHits:
                if v['attributeId'] == 'core.classType' and v['value'] == 'com.infa.ldm.profiling.DataDomainGroup':
                    ...
                    dataDomainGroupReq = session.get(
                        dataDomainGroupUrl, headers=EDC_headers, auth=EDC_Auth, verify=False)
                ...
                if v['attributeId'] == 'core.classType' and v['value'] == 'com.infa.ldm.profiling.DataDomain':
                    ...
                    dataDomainReq = session.get(
                        dataDomainURL, headers=EDC_headers, auth=EDC_Auth, verify=False)
...
def datacolumnSCV():
    for val in dataDomainNames:
        ...
        for (finalId, val2) in val.items():
            ...
            dataColumnUrl = EDC_URL_REST_2 + '/1/catalog/data/search?q=(com.infa.ldm.profiling.dataDomainsAccepted:'+ val2 +'%20AND%20core.name:*)&qf=com.infa.ldm.profiling.dataDomainsAccepted&fq=com.infa.ldm.profiling.dataDomainsAccepted:'+ val2 +'&related=false&includeRefObjects=false&offset=0&pageSize=1'
            dataColumnRes = ''
            hits = ''
            totalPage = ''

            dataColumnReq = session.get(
                dataColumnUrl, headers=EDC_headers, auth=EDC_Auth, verify=False)
            if dataColumnReq.status_code == 200:
                if not dataColumnReq.content:
                    ...
                    while totalPage > pageSizeColOffset:
                            ...
                            newUrlMain = EDC_URL_REST_2 + '/1/catalog/data/search?q=(com.infa.ldm.profiling.dataDomainsAccepted:'+ val2 +'%20AND%20core.name:*)&qf=com.infa.ldm.profiling.dataDomainsAccepted&fq=com.infa.ldm.profiling.dataDomainsAccepted:'+ val2 +'&related=false&includeRefObjects=false&offset='+str(pageSizeColOffset)+'&pageSize=' + str(pageSizeDD)
