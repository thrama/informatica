from props.paramsAxon import baseurl, chars_to_remove, facetPageLimit
from props.utils import BeautifulSoup, json, logging, os, pd, requests
...
def getSystemResources(searchUrl, infaTokenValue, headerValue, sessionCookies):
    ...
    r = requests.post(searchUrl, headers=headerValue, cookies=sessionCookies, stream=True, data=body)
    ...
    if r.status_code == 200:
        ...
        if not mainItems:
            ...
            for x in mainItems:
                ...
                newurl = f"{baseurl}/api/v1/referencelinks?sourceIds[]={reliD}&entityType=Axon\\AppBundle\\Entity\\System&systemIdentifier=Axon&relationshipNames[]=AxonSystemToEICResource"

                systemR = requests.get(newurl, headers=headerValue, cookies=sessionCookies, stream=True)
        ...
        while totalHits > limitValue:
            ...
            rPage = requests.post(
                searchUrl, headers=headerValue, cookies=sessionCookies, stream=True, data=newFacet
            )
            ...
            if rPage.status_code == 200:
                ...
                if not mainItems:
                    ...
                    for x in mainItems2:
                        reliD = x.get("id")
                        # set token autherization
                        newurl2 = f"{baseurl}/api/v1/referencelinks?sourceIds[]={reliD}&entityType=Axon\\AppBundle\\Entity\\System&systemIdentifier=Axon&relationshipNames[]=AxonSystemToEICResource"

                        systemR2 = requests.get(newurl2, headers=headerValue, stream=True)
                        if systemR2.status_code == 200:
                            ...
                            logging.error(
                                f"Resource ID {reliD} pagination failed with status code: {str(systemR.status_code)}"
                            )
