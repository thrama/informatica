...
from props.paramsAxon import chars_to_remove, facetPageLimit
from props.utils import BeautifulSoup, datetime, dt_string, json, logging, os, pd, requests, time
...
def noRelationFacets(baseurl, infaTokenValue, headerValue, sessionCookies):
    ...
    for facet in facets:
        ...
        r = requests.post(baseurl, headers=headerValue, cookies=sessionCookies, stream=True, data=bodyData)
        # db data to be saved
        ...
        if r.status_code == 200:
            ...
            getOrgUnitItems = json_data.get("mainObject")  # get mainObject with items
            ...
            if not mainItems:
                ...
                exportData = createArray(mainItems, toExport)
                ...
                while totalHits > limitValue:
                    ...
                    r2 = requests.post(baseurl, headers=headerValue, cookies=sessionCookies, stream=True, data=newFacet)
                    ...
                    if r2.status_code == 200:
                        ...
                        exportData = createArray(offsetItems, toExport)
                    ...
                    if nameFacete != "ATTRIBUTE":
                        ...
                        for column in df[0:-1].columns:
                            ...
                            for i in chars_to_remove:
                                ...
                                # save to csv: facetname + current date
                                df.to_csv(
                                    os.path.join(f"../AXON/{nameFacete}.csv"),
                                    sep=";",
                                    columns=columns,
                                    index=False,
                                )
        ...
        axon_facets(nameFacete, response_code, response_size, response_time, downloaded, starting_time, ending_time)
