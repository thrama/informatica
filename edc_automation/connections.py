...
class Connections:
    ...
    def getSecureConnectionName(connectionName, dsn):
        ...
        if len(connectionName) > 32:
            if len(dsn) > 32:
                print(f"Error: SecureConnectionName [{connectionName}] and DSN [{dsn}] are longer than 32 characters. I will not create the connection.")
                logging.error(f"SecureConnectionName [{connectionName}] and DSN [{dsn}] are longer than 32 characters. I will not create the connection.") # log
                return False
                ...
                print(f"Error: SecureConnectionName [{connectionName}] is longer than 32 characters, but you did not provide an alternative for the DSN. I will not create the connection.")
                logging.error(f"SecureConnectionName [{connectionName}] is longer than 32 characters, but you did not provide an alternative for the DSN. I will not create the connection.") # log
                return False
    ...
    def createDB2(self, arr, tech):
        ...
        for i in arr:
            ...
            command = self.createCommand(i[2], tech, i[6], i[7], options) # connectionName, tech, connectionUserName, connectionPassword, options
            print(command)
    ...
    def createSQLS(self, arr, tech):
        ...
        for i in arr:
            ...
            command = self.createCommand(i[2], tech, i[5], i[6], options) # connectionName, tech, connectionUserName, connectionPassword, options)
            print(command)
            ...
            try:
                ...
                odbcFile.appendConSQLSrv(dsn, i[11], globalparams.ldapDomain, i[8], i[10], truncate) # connectionName, database, ldapDomain, host, port, truncate
                truncate = False
